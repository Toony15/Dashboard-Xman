...
def satisfaction_page():
    ...
    def read_and_merge(files):
        ...
        for uploaded_file in files:
            ...
            # Ambil Event dan Unit dari baris info di bagian atas file
            event = ""
            unit = ""
            for i in range(min(5, len(raw))):
                cell = str(raw.iloc[i, 0]) if pd.notna(raw.iloc[i, 0]) else ""
                if "Nama Pelatihan" in cell:
                    event = cell.split(":", 1)[-1].strip()
                elif "School" in cell:
                    unit = cell.split(":", 1)[-1].strip()

            # Deteksi Quarter dari nama file (format: Q<digit><tahun>, misal Q32026 = Q3 2026)
            quarter_match = re.search(r'Q(\d)(\d{4})', uploaded_file.name)
            quarter = f"Q{quarter_match.group(1)}" if quarter_match else ""

            # Telusuri baris demi baris untuk menemukan blok data tiap instruktur
            records = []
            current_expert = ""
            in_block = False
            ...
            for i in range(len(raw)):
                ...
                col_e = str(row[4]).strip().upper() if len(row) > 4 and pd.notna(row[4]) else ""
                col_f = str(row[5]).strip().upper() if len(row) > 5 and pd.notna(row[5]) else ""

                # Judul blok instruktur, contoh: "LIM 1 - NAMA INSTRUKTUR"
                if col_a.upper().startswith("LIM") and col_e != "QUESTION":
                    current_expert = re.sub(r'^LIM\s*\d*\s*-\s*', '', col_a, flags=re.IGNORECASE).strip()
                    continue

                # Baris header data mentah blok ini
                if col_e == "QUESTION" and col_f == "ANSWER":
                    in_block = True
                    continue

                # Baris "AVERAGE" menandai akhir blok
                if col_a.upper() == "AVERAGE":
                    in_block = False
                    continue
