...
def _norm_text(s):
    """Normalize text: NFKD, remove punctuation, collapse spaces, lowercase."""
...
def variation_page():
    ...
    Adds a quarter filter (Q1..Q4) and uses DB-derived max total point as default when available.
    """
    ...
    # load mapping (built-in used when no file)
    ...
    # minimal styling and number formatting
    ...
    # If data came from DB, suggest using the observed max total_point for normalization
    db_max_point = None
    if source == "From Data Base" and not summary.empty:
        try:
            db_max_point = float(summary["total_point"].max())
        except Exception:
            db_max_point = None

    default_max = float(db_max_point) if db_max_point is not None and db_max_point > 0 else 25.0
    if db_max_point is not None:
        st.info(f"Total point tertinggi untuk dataset (dipakai sebagai default): {db_max_point}")

    max_point = st.number_input("Total Point Tertinggi (untuk normalisasi skor)", value=default_max, step=1.0)
    ...
    denom = float(max_point) if float(max_point) != 0 else 1.0
    summary["score_percent"] = (summary["total_point_clamped"] / denom * 100).clip(0, 100).round(2)
    ...
    # optional OpenAI/GenAI summary for top3
    try:
